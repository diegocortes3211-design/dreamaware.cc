...
export class SnapshotCache {
    ...
    getStats() {
        ...
        const compressionRatio = this.cache ? (this.cache.data.length / this.cache.uncompressed * 100) : null;
        return {
            cached: !!this.cache,
...
export class StreamServer {
    ...
    handleSnapshotRequest(conn: WebSocketConnection, clientTickId: number = 0) {
        ...
        if (!snapshot) {
            ...
            conn.sendQ.push({ type: 'snapshot_pending', message: 'Snapshot building, sending deltas...', weight: 1 });
            return false;
        }
